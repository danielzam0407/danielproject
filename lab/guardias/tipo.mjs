...
const perfil=join(tmpdir(),'nerv-t-'+process.pid), puerto=9000+(process.pid%900);
const nav=spawn(EDGE,['--headless=new','--remote-debugging-port='+puerto,'--user-data-dir='+perfil,
 '--no-first-run','--hide-scrollbars','--window-size=1440,900','about:blank'],{stdio:'ignore'});
let ws;
try{
 ...
 const r=await s('Runtime.evaluate',{returnByValue:true,expression:`(() => {
  const cara=document.querySelector('.v4');
  const tam={}, peso={}, fam={};
  cara.querySelectorAll('*').forEach(n=>{
    ...
    tam[px]=(tam[px]||0)+1;
    peso[cs.fontWeight]=(peso[cs.fontWeight]||0)+1;
    const f=(cs.fontFamily||'').split(',')[0].replace(/["']/g,'');
    fam[f]=(fam[f]||0)+1;
  });
  const orden=o=>Object.entries(o).sort((a,b)=>b[1]-a[1]).map(([k,v])=>k+'×'+v).join(' ');
  return JSON.stringify({tamanos:orden(tam), pesos:orden(peso), familias:orden(fam)});
 })()`});
 console.log('  ' + r.result.value.replace(/","/g,'\n   "'));
