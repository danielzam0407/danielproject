...
const perfil=join(tmpdir(),'nerv-s-'+process.pid), puerto=9000+(process.pid%900);
 ...
 for (const lang of ['es','en']) {
  ...
  const r=await s('Runtime.evaluate',{returnByValue:true,awaitPromise:true,expression:`(async () => {
   try{
    ...
    const dem=document.querySelector('.v4-demo');
    const out=[];
    for (let k=0;k<bs.length;k++){
      bs[k].click(); await new Promise(r=>setTimeout(r,420));
      out.push(bs[k].textContent+': h1='+Math.round(h1.getBoundingClientRect().height)+
        ' demoY='+Math.round(dem.getBoundingClientRect().top)+
        ' pag='+document.documentElement.scrollHeight);
    }
    return out.join('  //  ');
