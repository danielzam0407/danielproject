...
const perfil=join(tmpdir(),'nerv-dx-'+process.pid), puerto=9000+(process.pid%900);
...
try{
 ...
 cdp.al('Runtime.exceptionThrown',p=>errores.push('EXC '+(p.exceptionDetails&&p.exceptionDetails.text)));
 cdp.al('Log.entryAdded',p=>{ if(p.entry.level==='error') errores.push('LOG '+p.entry.text.slice(0,110)); });
 ...
 const r=await s('Runtime.evaluate',{returnByValue:true,expression:`(() => {
   ...
   [...document.body.children].forEach(n=>{
     ...
     if(r.height>4) out.push({tag:n.tagName.toLowerCase(), cls:(n.className||'').toString().slice(0,26),
       id:n.id||'', top:Math.round(r.top+scrollY), h:Math.round(r.height), pos:cs.position});
   });
   return JSON.stringify(out.slice(0,14));
 })()`});
 console.log('  primeros hijos del body con altura:');
 JSON.parse(r.result.value).forEach(o=>console.log(`    ${String(o.top).padStart(5)}  h=${String(o.h).padStart(5)}  ${o.tag}${o.id?'#'+o.id:''}${o.cls?'.'+o.cls:''}  (${o.pos})`));
