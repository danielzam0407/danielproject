...
const perfil=join(tmpdir(),'nerv-esc-'+process.pid), puerto=9000+(process.pid%900);
...
try{
 ...
 await s('Page.addScriptToEvaluateOnNewDocument',{source:`try{localStorage.setItem('dp-lang','es')}catch(e){}`});
 ...
 const r=await s('Runtime.evaluate',{returnByValue:true,expression:`(() => {
   const t=document.querySelector('.v4').innerText;
   return JSON.stringify({
     es_titular: t.includes('Hacemos sitios'),
     es_lede: t.includes('sin plantillas'),
     es_pasos: t.includes('Me cuentas'),
     es_cierre: t.includes('Pregúntale'),
     quedan_en_ingles: ['We make websites','What we do','The work','You tell me','Ask it'].filter(x=>t.includes(x))
   }); })()`});
 console.log('  ES:', r.result.value);
