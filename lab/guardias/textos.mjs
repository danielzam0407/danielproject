import { spawn } from 'node:child_process'; import { rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path'; import { tmpdir } from 'node:os';
const EDGE='C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe';
const url=process.argv[2], salida=process.argv[3]; const dormir=m=>new Promise(r=>setTimeout(r,m));
...
const perfil=join(tmpdir(),'nerv-tx-'+process.pid), puerto=9000+(process.pid%900);
...
try{
 ...
 const r=await s('Runtime.evaluate',{returnByValue:true,expression:`(() => {
   const cara=document.querySelector('.v4'); const out=[];
   const p=document.createTreeWalker(cara,NodeFilter.SHOW_TEXT);
   let n; while((n=p.nextNode())){ const v=n.nodeValue.trim();
     if(v.length>1 && !/^[\d\s:·\[\]]+$/.test(v)) out.push(v); }
   return JSON.stringify([...new Set(out)]);
 })()`});
 await writeFile(salida, r.result.value, 'utf8');
 console.log('  nodos de texto extraidos:', JSON.parse(r.result.value).length);
